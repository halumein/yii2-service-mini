...
usesgraphcrt.servicemini = {
    init: function() {
        ...
        $(document).on('chooseUserToOrder', function(event, id) { usesgraphcrt.servicemini.getProperties(id); })
        ...
        $(document).on('submit', '.order-create-container form', function() {
            ...
            //usesgraphcrt.servicemini.clearServiceOrder();
        });

        $(document).on('blur', '#service-ident', function() {
            $($(this).data('field-selector')).val($(this).val());
            usesgraphcrt.servicemini.searchClientByIdent($(this).val());
        });
        ...
        $(document).on('click', '.calculate-service-model', function() {
            var self = $(this),
                url = self.data('url');
            usesgraphcrt.servicemini.getCalculatedService(url);
...
//        });
    ...
    searchClientByIdent: function(ident) {
        jQuery.post(usesgraphcrt.servicemini.searchClientByIdentUrl, {ident: ident},
            function(json) {
    ...
    getProperties: function(clientId) {
        ...
        jQuery.get(usesgraphcrt.servicemini.propertyListUrl, {clientId: clientId},
            function(json) {
                if(json.result == 'success') {
